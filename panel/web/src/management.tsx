...
function PlayerRow({ player, onSelect }: { player: Player; onSelect: (action: PlayerAction) => void }): ReactNode {
  ...
  const avatarPath = player.steamid && !player.isBot ? `/api/players/${encodeURIComponent(player.steamid)}/avatar` : null;
  return <Card className="panel-card player-row"><div className="player-identity"><div className="avatar player-avatar">{player.name.slice(0, 2).toUpperCase()}{avatarPath && <img src={avatarPath} alt="" loading="lazy" referrerPolicy="no-referrer" onError={(event) => { event.currentTarget.hidden = true; }} />}</div><div><h2>{player.name}</h2><span>#{player.userid} · {player.steamid ?? "Unauthenticated"}</span></div>{player.isAdmin && <Badge className="running">Admin</Badge>}{player.isBot && <Badge variant="secondary">Bot</Badge>}</div><dl><div><dt>Ping</dt><dd>{player.ping ?? "—"}</dd></div><div><dt>Time</dt><dd>{player.time ?? "—"}</dd></div><div><dt>State</dt><dd>{player.state}</dd></div></dl><div className="player-action"><NativeSelect value={action} onChange={(event) => setAction(event.target.value as PlayerAction)}>{PLAYER_ACTIONS.map((item) => <option key={item}>{item}</option>)}</NativeSelect><Button variant="outline" onClick={() => onSelect(action)}>Review action</Button></div></Card>;
}
